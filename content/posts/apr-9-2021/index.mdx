...
##### 6:20
I reviewed the code and fixed some things before I think about getting started on the feature. I added Redux selector in the Anonymous portal so that it loads faster depending on whether the user is admin or not. I also made the admin checking more concise on other parts of the app with the `?.` selector. Nice!
...
- The admin by default sees the people that haven't received a reply.
...
- The original admin may get out of sync due to the actions taken by the other one, so will need database listeners on each side to listen for changes.
